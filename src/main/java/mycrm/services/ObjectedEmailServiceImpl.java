...
public class ObjectedEmailServiceImpl implements ObjectedEmailService {
    ...
    @Override
    public void sendObjectedEmail(EnergyContract contract, String emailAddress) {
        if (!StringUtils.hasText(emailAddress)) {
            logger.info("No email address for manual Objected email");
            return;
        }
        ...
        logger.info("Sending Objected email.");
        boolean isEmailSent = sendObjectedEmailFromTemplateService(emailAddress, customerReference, accountNumber);
        if (isEmailSent) {
            emailHistoryService.saveEmailHistory(emailAddress, contract, EmailProcess.MANUAL.getName(), emailType);
        }
        logger.info("Objected email sent.");
    }
    ...
    private void collateCustomerDetailsFromContract(EnergyContract contract, String process) {
        ...
        customerEmails.forEach(email -> {
            ...
            logger.info("Sending Objected email.");
            boolean isEmailSent = sendObjectedEmailFromTemplateService(email, customerReference, accountNumber);
            if (isEmailSent) {
    ...
    private boolean sendObjectedEmailFromTemplateService(String emailAddress, String customerReference, String accountNumber) {
        try {
            return emailTemplateService.sendObjectedEmail(customerReference, emailAddress, accountNumber);
        } catch (Exception e) {
            logger.error("Failed to send Objected email to: {} {}", emailAddress, e.getMessage());
            return false;
        }
    }
}
